...
#main funciton ------------------------------------------------------------------------------------------
def mainmedium():#looping for the screen to be displayed continously
...
#ufoX position in the screen; its not inside for bcs if not it cant change position if key is up
    ...
    NOE = 8 #number of enemies 
    #adding skull to the list 
    for z in range(NOE):
        skullImg.append(pygame.image.load("tako.png"))
    ...
    def tryagain_text():
        if button == 0:
            ...
            text = TA_font.render(mainmedium(),1,(0,0,0))
