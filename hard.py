...
def shoot_star(x,y):
    myscreen.blit(starImg,(x + 16,y + 1))#to make it appear on the middle of the UFO
#-------------------------------------------------------------------------------------------------------
...
#main funciton ------------------------------------------------------------------------------------------
def mainhard():#looping for the screen to be displayed continously
...
#ufoX position in the screen; its not inside for bcs if not it cant change position if key is up
    ...
    NOE = 10 #number of enemies 
    #adding skull to the list 
    for z in range(NOE):
        skullImg.append(pygame.image.load("monster.png"))
        skullX.append(random.randint(0,735))
        skullY.append(random.randint(50, 150))
        skullX_change.append(3)
        skullY_change.append(40)
    ...
    def tryagain_text():
        if button == 0:
            ...
            text = TA_font.render(mainhard(),1,(0,0,0))
    ...
    #skull movement
        for z in range(NOE):
            ...
            if skullY[z]>360:
                ...
                break
    #skull boundaries
            skullX[z] += skullX_change[z]#to let our program now which skullchange are we talking about
            if skullX[z] <= 0:
                skullX_change[z] = 4
                skullY[z] += skullY_change[z]
            elif skullX[z] >= 736:
                skullX_change[z] = -4
                skullY[z] += skullY_change[z]
        #crash
    ...
    #star movement
        ...
        if star_condition is "shoot":
            ...
            starY -= starY_change #goes to the top of the screen
    #---------------------------------------------------------------------------------------------------
